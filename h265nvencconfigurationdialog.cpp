...
#include "ui_h265nvencconfigurationdialog.h"

h265NVENCConfigurationDialog::h265NVENCConfigurationDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::h265NVENCConfigurationDialog)
{
    ui->setupUi(this);
    ...
    config = new ConfigurationManager("/assets/h265NVENC.conf");
    ...
    ui->h265_nvenc_encoding_speed_combobox->addItem(ENCODER_SPEED_LLHP_LABEL);
    ui->h265_nvenc_encoding_speed_combobox->addItem(ENCODER_SPEED_LLHQ_LABEL);

    ui->h265_nvenc_process_priority_combobox->addItems(SYS_PRIORITY_OP_LIST);

    ui->h265_nvenc_frame_threads_combobox->addItems(FRAME_THREADS_OPT_LIST);
    ui->h265_nvenc_pool_threads_combobox->addItems(POOL_THREADS_OPT_LIST);

    ui->h265_nvenc_rate_control_combobox->addItems(RATE_CONTROL_OPT_LIST);

    ui->h265_nvenc_fec_percentage_combobox->addItems(FEC_OPT_LIST);

    setLoadedValues();
}

h265NVENCConfigurationDialog::~h265NVENCConfigurationDialog()
{
    delete ui;
}
...
void h265NVENCConfigurationDialog::setLoadedValues() {
    if(config->getKey("nv_preset") == ENCODER_SPEED_LLHP) {
        ui->h265_nvenc_encoding_speed_combobox->setCurrentIndex(0);
        entries_snapshot.insert("nv_preset", ENCODER_SPEED_LLHP);
    }
    else if(config->getKey("nv_preset") == ENCODER_SPEED_LLHQ) {
        ui->h265_nvenc_encoding_speed_combobox->setCurrentIndex(1);
...
void h265NVENCConfigurationDialog::on_h265_nvenc_cancel_button_clicked()
{
    this->hide();
    setLoadedValues();
}

void h265NVENCConfigurationDialog::on_h265_nvenc_ok_button_clicked()
{
    //encoder preset
    QString selected_encoder_preset_label = ui->h265_nvenc_encoding_speed_combobox->currentText();
    ...
    /*System priority*/
    QString selected_system_priority_label = ui->h265_nvenc_process_priority_combobox->currentText();
    ...
    /*Frame threads*/
    QString selected_frame_threads = ui->h265_nvenc_frame_threads_combobox->currentText();
    config->setEntry("min_threads", selected_frame_threads);
    /*Pool threads*/
    QString selected_pool_threads = ui->h265_nvenc_pool_threads_combobox->currentText();
    config->setEntry("pools", selected_pool_threads);

    /*rate control*/
    QString selected_rate_control_label = ui->h265_nvenc_rate_control_combobox->currentText();
    ...
    /*fec percentage*/
    QString selected_fec = ui->h265_nvenc_fec_percentage_combobox->currentText();
